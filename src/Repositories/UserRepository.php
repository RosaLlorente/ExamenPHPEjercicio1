...
class UserRepository{
    ...
    public function create(User $user) : bool
    {
        try{
            ...
            $sql = $this->db->prepare('INSERT INTO usuarios (nombre,apellidos,telefono,DNI,compania,email,password) VALUES (:nombre,:apellidos,:telefono,:dni,:compania,:email,:password)');
            $sql->bindValue(':nombre', $user->getNombre(), PDO::PARAM_STR);
            $sql->bindValue(':apellidos', $user->getApellido(), PDO::PARAM_STR);
            $sql->bindValue(':telefono', $user->getTelefono(), PDO::PARAM_INT);
            $sql->bindValue(':dni', $user->getDni(), PDO::PARAM_STR);
            $sql->bindValue(':compania', $user->getCompania(), PDO::PARAM_STR);
            $sql->bindValue(':email', $user->getEmail(), PDO::PARAM_STR);
